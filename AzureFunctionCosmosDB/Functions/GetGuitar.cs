...
namespace AzureFunctionCosmosDB.Functions
{
    ...
    { 
        ...
        public GetGuitar(CosmosClient cosmosClient)
        {
            this.cosmosClient = cosmosClient;
        }
  
        [FunctionName("get-guitar")]
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "get-guitar/{id}")] HttpRequest req,
            ILogger log, string id)
        {

            var container = this.cosmosClient.GetContainer(Environment.GetEnvironmentVariable("DataBase"), "Guitars");

            var sqlQueryText = $"SELECT * FROM Guitars where Guitars.id = '{id}'";
